              ...
              {isExpanded && (
                <div className="p-3 sm:p-4 bg-white space-y-4">
                  ...
                  {group.rows.map((row, rowIdx) => {
                    const isCompleted = (!row.status || row.status === 'completed') && 
                      (row.recommendation.trim() === '' || row.recommendation.toLowerCase().includes('không phát sinh kiến nghị'));
                    const isPending = row.status === 'pending';
                    const isInProgress = row.status === 'in_progress' || (!isCompleted && !isPending);
                    const rowHasIssue = isPending || isInProgress;
                    ...
                    return (
                      ...
                      >
                        <div className="flex flex-wrap items-center justify-between pb-2.5 mb-2.5 border-b border-slate-200/60 gap-2">
                          <div className="flex items-center gap-2">
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            <div className="flex items-center bg-slate-100 p-0.5 rounded-lg text-[11px]">
                              <button
                                type="button"
                                onClick={() => {
                                  handleRowChange(originalGroupIndex, rowIdx, 'status', 'completed');
                                  handleResetRowStandard(originalGroupIndex, rowIdx);
                                }}
                                title="Đạt chuẩn - Không có kiến nghị"
                              ...
                              <button
                                type="button"
                                onClick={() => handleRowChange(originalGroupIndex, rowIdx, 'status', 'in_progress')}
                                title="Có kiến nghị / Đang theo dõi xử lý"
                                className={`px-2 py-0.5 rounded-md font-semibold transition ${
                                  isInProgress
                              ...
                              <button
                                type="button"
                                onClick={() => handleRowChange(originalGroupIndex, rowIdx, 'status', 'pending')}
                                title="Tồn tại chưa khắc phục - Tự động chuyển tiếp sang tháng sau"
                                className={`px-2 py-0.5 rounded-md font-semibold transition ${
                                  isPending
