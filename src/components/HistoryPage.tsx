...
export const HistoryPage: React.FC<HistoryPageProps> = ({
  ...
  const filteredReports = useMemo(() => {
    return reports
      ...
      .sort((a, b) => {
        // Compare by thang_nam (format: MM/YYYY)
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Top Banner for Colleagues & Permission Notification */}
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-base sm:text-lg font-bold text-slate-900">
                Lịch sử biên bản kiểm tra ATVSLĐ các tháng
              </h2>
              <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-blue-50 text-blue-700 border border-blue-200">
                {reports.length} biên bản
              </span>
            </div>
            <p className="text-xs text-slate-500 mt-0.5 font-medium">
              VHIALY  •  Công ty Thủy điện Ialy • Tra cứu biên bản kiểm tra ATVSLĐ các tháng
            </p>
          </div>
        </div>
          ...
          {isAdmin ? (
            <div className="flex items-center gap-2">
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold bg-emerald-50 text-emerald-800 border border-emerald-200">
                ...
                Quyền Admin (Được phép soạn thảo & sửa)
              </span>
              <button
                id="btn-history-create-new"
                onClick={onCreateNewReport}
                className="inline-flex items-center gap-1 px-3.5 py-1.5 text-xs font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-xl transition shadow-xs"
              >
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto justify-between sm:justify-end">
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium bg-slate-100 text-slate-700 border border-slate-200">
                ...
                Chế độ Đồng nghiệp (Chỉ xem)
              </span>
              <button
                id="btn-history-unlock-admin"
              ...
              >
                ...
                <span>Đăng nhập Admin để soạn thảo</span>
              </button>
            </div>
          )}
        </div>
      ...
      <div className="bg-white rounded-2xl p-4 border border-slate-200/80 shadow-xs flex flex-col sm:flex-row items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto justify-between sm:justify-end overflow-x-auto pb-1 sm:pb-0">
          ...
          <button
            ...
            className="px-3 py-1.5 text-xs font-medium text-slate-700 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-xl transition flex items-center gap-1.5 whitespace-nowrap"
          >
      ...
      {filteredReports.length === 0 ? (
        <div className="bg-white rounded-2xl p-12 text-center border border-slate-200">
          ...
          <h3 className="text-sm font-bold text-slate-700">Không tìm thấy biên bản nào phù hợp</h3>
          <p className="text-xs text-slate-500 mt-1">
            Vui lòng thử tìm với từ khóa khác hoặc bỏ chọn bộ lọc năm.
          </p>
            ...
            return (
              ...
              >
                ...
                <div className="p-3.5 bg-slate-50 border-t border-slate-100 flex items-center justify-between gap-2">
                  ...
                  {/* Right: Admin Actions (Only if admin) */}
                  {isAdmin ? (
                    <div className="flex items-center gap-1 border-l pl-2 border-slate-200">
                      ...
                      <button
                        ...
                        title="Xóa biên bản này khỏi kho lưu trữ"
